...
export class Profile implements OnInit  {  
     ...
     next: (res: any) => {
           ...
           const formData = {
            ...
            //  this.qrcodeurl = img.src;
             ...
             this.mfa = false;
              this.qrcodeurl = "/static/images/qrcode.png";
            }
            setTimeout(() => {
              this.profileMsg.set('');
            }, 1000);
  ...
  passwordChange() {
    this.ngZone.run(() => {
      ...
      this.profileService.sendNewpasswordRequest(formData, this.jwttoken).subscribe({
        next: (res: any) => {
  ...
  submitProfileForm() {
    this.ngZone.run(() => {
        ...
        this.profileService.sendProfileRequest(jsonData, this.jwttoken).subscribe({
          next: (res: any) => {
            this.profileMsg.set(res.message);
            setTimeout(() => {
              this.profileMsg.set('');
